	...
	<div class="card-header">
		...
		<table id="datatablesSimple">
			...
			<tbody>
				...
				$sql = "SELECT *,servicebooking.id as bid from servicebooking join servicecenters on servicecenters.id=servicebooking.servicePoint";
				...
				if ($row > 0) {
					foreach ($temp as $result) {				?>
						<tr>
							...
							<td><?php echo htmlentities($result['servicecenteraddress']); ?></td>
							<td><?php echo htmlentities($result['serviceDate'] . "/" . $result['servicetime']); ?></td>

							<td><?php echo htmlentities($result['postingDate']); ?></td>


							<td><a href="http://localhost/Garage92/admin1/?page=booking/all-bookings.php/booking-details.php?bid=<?php echo htmlentities($result['bid']); ?>&&bookingid=<?php echo htmlentities($result['bookingId']); ?>">View</a>
							</td>
						<?php } ?>
						</tr>
					<?php } else { ?>
